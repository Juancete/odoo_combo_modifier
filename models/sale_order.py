    ...
    def _create_combo_lines(self):
        ...
        for combo_line in self.product_id.product_tmpl_id.combo_line_ids:
            self.env['sale.order.combo.line'].create({
                ...
                'quantity': combo_line.quantity * self.product_uom_qty,
    ...
    def _update_combo_quantities(self):
        ...
        for combo_line in self.combo_component_lines:
            if combo_line.included:
                combo_line.quantity = combo_line.original_quantity * self.product_uom_qty
...
class SaleOrderComboLine(models.Model):
    _name = 'sale.order.combo.line'
    _description = 'Línea de Componente de Combo en Pedido'
    _order = 'sequence, id'
